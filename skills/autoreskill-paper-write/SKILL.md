...
- Build `paper/WRITING_QUALITY_PROFILE.json` before major prose integration or review repair. It is a paragraph-logic, survey-target, figure/table, and claim-strength dashboard; it is not a stage authority.
...
- Experiment-backed numerical claims must follow the locked `metric_policy` from analysis. For multi-metric protocols, state or table the full metric vector and composite/stress rule; do not turn an isolated favorable component such as `New` into a general improvement claim when other required metrics regress or are missing.
...
- Use paragraph logic deliberately: claim-evidence-implication for main claims, compare-contrast for prior/method tradeoffs, concession-rebuttal for limitations, and funnel structure for introduction framing. The writing profile reports missing patterns as warnings so writers can repair prose without weakening evidence rules.
- For survey-mode manuscripts, citation counts, table counts, figure counts, and taxonomy coverage are profile warnings, not default method-paper blockers. Keep survey quantity heuristics separate from claim verification.
...
Read `references/story_contract_schema.md`, `references/venue_template_mapping.md`, `references/writing_review_enhancement.md`, and venue-specific notes when available.
