...
- Before launching any baseline/proposed command, also run the sibling skill helper `../autoreskill-implement-experiment/scripts/baseline_clone_lint.py --project <project-root>`. Do not launch if the baseline is not a clone/worktree or if the proposed implementation lacks patch proof against that clone.
