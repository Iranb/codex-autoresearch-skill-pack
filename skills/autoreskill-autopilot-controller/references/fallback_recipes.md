...
- `dry_run_fail`: repair the same operational signature at most twice by default; then shrink to baseline-only smoke, change the implementation route, or roll back to `experiment_plan` without weakening the hypothesis.
