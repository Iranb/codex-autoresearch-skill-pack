...
- `ideation`: `.autoreskill/ideation/PRE_IDEA_EVIDENCE_GATE.json` + `.autoreskill/ideation/INNOVATION_SLOT_MAP.json` + `.autoreskill/ideation/EXPERIMENT_IDEA_POOL.json` + `.autoreskill/user_view/innovation_story/00_STORYLINE_DESIGN.md`; idea generation must follow target-domain, near-neighbor, and far-neighbor broad PaperNexus discovery with `pre_idea_discovery_config_lint.py` passing, active screening breadth, `PAPER_SELECTION_SCORECARD.json`, `GRAPH_IMPORT_PLAN.json`, `IMPORT_WORKFLOW_STATUS.json` for selected import tasks, PaperNexus graph/material split-reading evidence, and a committed `papernexus/proposal_graph_session.json` when `proposal_graph_session` is available. The target-domain lane anchors the task, closest priors, baselines, datasets, metrics, protocols, and overlap risks; near/far-neighbor lanes should supply the primary method mechanisms, transfer bridges, or cross-lane recombinations for the main paper idea. The user-facing storyline must explain the reader belief shift, opening tension, hidden cause, method-as-resolution, proof ladder, figure story, and reviewer risk. The only exceptions are unavailable/unrecorded proposal graph support, diagnosis-only proposal sessions with explicit fallback boundary, or an explicit approved degraded gate with claim limits.
- `idea_gate` selected experiment idea: `.autoreskill/ideation/PRE_IDEA_EVIDENCE_GATE.json` + `.autoreskill/ideation/IDEA_NOVELTY_VENUE_SCORECARD.json` + `.autoreskill/ideation/EXPERIMENT_IDEA_POOL.json` + `.autoreskill/user_view/innovation_story/00_STORYLINE_DESIGN.md`; approved degraded gates may pass only as speculative idea selection with evidence debt routed to `experiment_plan`.
- `experiment_plan`: `.autoreskill/orchestrator/INNOVATION_PACKET.json` + `.autoreskill/user_view/innovation_story/{00_STORYLINE_DESIGN.md,01_METHOD_INNOVATION_STORY.md,02_CLAIM_EVIDENCE_MAP.md}`; when the selected idea cites a proposal graph session, the packet must retain the proposal manifest path, committed subgraph id, controller trace paths, and proposal evidence export path. The reviewed plan must preserve the neighbor/cross-lane primary method source, separately close target-domain overlap risk before launch, and update the user-facing story so it explains how the method grows from current-field pressure plus near/far-neighbor or cross-lane transfer.
