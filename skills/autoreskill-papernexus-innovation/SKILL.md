...
- During ideation, always run broad `literature_discovery` for three lanes: `target_domain`, `near_neighbor`, and `far_neighbor`. The first broad pass should use async `submit -> progress -> report` and can be metadata-only, but it must use deep/recall-oriented planning and expansion defaults rather than quick metadata defaults. Synchronous `search` is reserved for small targeted lookups. `pre_idea_discovery_config_lint.py` must pass before idea generation. Idea generation is blocked until high-signal eligible papers have been imported, supplemented, or split-read through PaperNexus, or until an explicit blocker/degraded approval is recorded.
...
- Treat PaperNexus import as asynchronous queue work. Use `import_workflow queue_progress/status/wait` for every actionable `GRAPH_IMPORT_PLAN` row with `import_action=import/supplement`, keep progressive batching enabled (`importBatchEnabled=true`, `importBatchInitialTasks=4`, `importBatchMaxTasks=16`, `importBatchProgressive=true`), and capture `IMPORT_WORKFLOW_STATUS.json` with planned/effective-planned/submitted/completed/authoritative-sync counts. When supported, request `processingProfile=fast-md-background-semantic`, `completionPolicy=graph-visible`, `importExecutionMode=dag`, `llmContextWindowTokens=1000000`, `llmExtractionStrategy=long-context-first`, `llmLongContextMaxPapersPerCall=10`, and bounded `llmBatchConcurrency`. A paper is graph-visible only after `status=completed` and `stage=completed`; graph-grounded evidence closure still requires authoritative graph sync complete/superseded/not-required and semantic readiness when semantic graph relations are used. Fast commit alone is not enough. Split-reading/material views can satisfy `material_view` rows only, never an actionable `import`/`supplement` graph-import row. If exact source discovery, OA/index checks, and PaperNexus `pdfUrl/sourcePath/serverFilePath` attempts are exhausted for selected rows with no server-acceptable full text, record them as `source_limited_exceptions` / `sourcepath_required` / `source_unavailable` with claim limits and continue the workflow; those exception rows are not graph-grounded evidence and must not support strong novelty, method, baseline, limitation, or citation claims until later imported.
...
python scripts/papernexus_feature_matrix.py --project <project-root> --callable unknown --operation research_material_pack --operation research_controller --operation proposal_graph_session --capability literature_discovery.submit --capability import_workflow.wait --capability fast-md-background-semantic --capability long-context-first
