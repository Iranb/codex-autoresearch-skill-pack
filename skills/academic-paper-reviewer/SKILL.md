...
For portable AutoResearch review gates, convert the review output into the canonical machine-readable findings file:

```bash
python scripts/review_findings_adapter.py --project <project-root> --input <review-output.md> --output .autoreskill/reviewer/REVIEW_FINDINGS.json
```

The adapter preserves raw input under `.autoreskill/reviewer/academic-paper-reviewer/raw/`, maps Critical/Major/Minor issues to critical/high/medium severities, and keeps open high/critical findings blocking until resolved, fixed, waived, or accepted as risk.
